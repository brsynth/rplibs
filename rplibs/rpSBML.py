...
from cobra.io.sbml import CobraSBMLError, validate_sbml_model
...
from rplibs.cobra_format import cobraize, uncobraize
...
class rpSBML:
    ...
    def getObjective(self, objective_id: str, objective_value: float) -> None:

        objective = self.getPlugin("fbc").getObjective(objective_id)

        self.checklibSBML(objective, "Getting objective " + str(objective_id))
    ...
    def setName(self, name):
        ...
        self.modelName = (
            self.modelName if self.modelName[0].isalpha() else "rp_" + self.modelName
        )
    ...
    ############################ QUERY #######################################
    ##########################################################################
    def search_compartment(self, compartment: str) -> libsbml.Compartment:
        ...
        for comp_synonym in comp_synonyms:
            for comp_model in comp_models:
                if (
                    comp_synonym == comp_model.getId().lower()
                    or comp_synonym == comp_model.getName().lower()
                ):
                    return comp_model
        return None

    def search_specie(self, specie: str) -> libsbml.Species:
        ...
        for spe in self.getModel().getListOfSpecies():
            if re.search(specie, spe.getId(), re.IGNORECASE) or re.search(
                specie, spe.getName(), re.IGNORECASE
            ):
                return spe
        return None

    def search_reaction(self, reaction: str) -> libsbml.Reaction:
        ...
        for rxn in self.getModel().getListOfReactions():
            if re.search(reaction, rxn.getId(), re.IGNORECASE) or re.search(
                reaction, rxn.getName(), re.IGNORECASE
            ):
                return rxn
        return None

    def has_compartment(self, compartment: str, strict: bool = False) -> bool:
    ...
    def has_specie(self, specie: str, strict: bool = False) -> bool:
    ...
    def has_reaction(self, reaction: str, strict: bool = False) -> bool:
    ...
    def mergeFiles(
        ...
        source_rpsbml = rpSBML(inFile=input_sbml, name="source", logger=logger)
        target_rpsbml = rpSBML(inFile=input_target, name="target", logger=logger)
    ...
    def merge(
        ...
        # Copy target rpSBML object into a new one so that
        # it can be modified and returned
        merged_rpsbml = rpSBML(rpsbml=model, logger=logger)
        ...
        species_pathway_model_transl, missing_species = merged_rpsbml.copySpecies(
            source_sbml=pathway, compartment_id=compartment_id
        )

        ## REACTIONS ###################################
        reactions_in_both = merged_rpsbml.copyReactions(
            source_sbml=pathway, species=species_pathway_model_transl
        )
        ...
        ## TITLES ######################################
        merged_rpsbml.copyTitles(source_sbml=pathway)
        ...
        if merged_rpsbml.checkSBML():
            return (merged_rpsbml, reactions_in_both, missing_species, compartment_id)
        else:
            logger.error("Merging rpSBML objects results in a invalid SBML format")
            return None, None, None, None
    ...
    @staticmethod
    def renameSpecies(rpsbml: "rpSBML", transl_dict_species: Dict[str, str]) -> None:
    ...
    def cobraize(rpsbml: "rpSBML") -> "rpSBML":
        return rpSBML.renameSpecies(
            ...
            transl_dict_species={
                specie.getId(): cobraize(specie.getId(), specie.getCompartment())
    ...
    def uncobraize(rpsbml: "rpSBML") -> "rpSBML":
        return rpSBML.renameSpecies(
            rpsbml=rpSBML(rpsbml=rpsbml),
            transl_dict_species={
                specie.getId(): uncobraize(specie.getId(), specie.getCompartment())
                for specie in list(rpsbml.getModel().getListOfSpecies())
            },
        )

    def enable_package(self, pkg: str, url: str) -> None:
        # ) -> libsbml.SBasePlugin:
    ...
    def copyUnitDefinitions(self, source_sbml: "rpSBML") -> None:

        source_sbml_doc = source_sbml.getDocument()
        self.logger.debug(f"source_sbml_doc: {source_sbml_doc}")

        target_unitDefID = [
            i.getId() for i in self.getModel().getListOfUnitDefinitions()
        ]

        for source_unitDef in source_sbml_doc.getModel().getListOfUnitDefinitions():

            if (
                source_unitDef.getId() not in target_unitDefID
            ):  # have to compare by ID since no annotation
                ...
                for source_unit in source_unitDef.getListOfUnits():
                    ...
                    rpSBML.checklibSBML(
                        target_unit.setKind(source_unit.getKind()),
    ...
    def copyCompartments(self, rpsbml: "rpSBML") -> None:
        ...
        for source_compartment in rpsbml.getModel().getListOfCompartments():
            ...
            if not source_annotation:
                self.logger.warning(
                    f"No annotation for the source of compartment {source_compartment.getId()}"
                )

            for target_compartment in self.getModel().getListOfCompartments():
                self.logger.debug(
                    f"source_compartment id: {source_compartment.getId()}; target_compartment id: {target_compartment.getId()}"
                )
                self.logger.debug(
                    f"source_compartment name: {source_compartment.getName()}; target_compartment name: {target_compartment.getName()}"
                )
                ...
                # then, compare by MIRIAM
                    ...
                    if not target_annotation:
                        self.logger.warning(
                            f"No annotation for the target of compartment: {target_compartment.getId()}"
                        )
                    elif rpSBML.compareMIRIAMAnnotations(
                        source_annotation, target_annotation, self.logger
                    ):
                        found = True
                        break
            ...
            if not found:
                ...
                rpSBML.checklibSBML(
                    target_compartment.setMetaId(source_compartment.getMetaId()),
                ...
                if source_compartment.getId() == target_compartment.getId():
                    ...
                    rpSBML.checklibSBML(
                        target_compartment.setId(source_compartment.getId()),
                        "setting target id",
                    )
                rpSBML.checklibSBML(
                    target_compartment.setName(source_compartment.getName()),
                    "setting target name",
                )
                rpSBML.checklibSBML(
                    target_compartment.setConstant(source_compartment.getConstant()),
                ...
                rpSBML.checklibSBML(
                    target_compartment.setSBOTerm(source_compartment.getSBOTerm()),
                    "setting target annotation",
                )

        # self.logger.debug('comp_source_target: '+str(comp_source_target))

    def copyParameters(self, source_sbml: "rpSBML") -> None:
        ...
        for source_parameter in source_sbml_doc.getModel().getListOfParameters():
            ...
            if source_parameter.getId() not in target_paramsID:
                ...
                rpSBML.checklibSBML(
                    target_parameter.setId(source_parameter.getId()),
                    "setting target parameter ID",
                )
                rpSBML.checklibSBML(
                    target_parameter.setSBOTerm(source_parameter.getSBOTerm()),
                    "setting target parameter SBO",
                )
                rpSBML.checklibSBML(
                    target_parameter.setUnits(source_parameter.getUnits()),
                    "setting target parameter Units",
                )
                rpSBML.checklibSBML(
                    target_parameter.setValue(source_parameter.getValue()),
                    "setting target parameter Value",
                )
                rpSBML.checklibSBML(
                    target_parameter.setConstant(source_parameter.getConstant()),
                    "setting target parameter ID",
                )

        # return target_paramsID

    def copyFBCGeneProducts(self, source_rpsbml: "rpSBML") -> None:
        ...
        for source_geneProduct in source_fbc.getListOfGeneProducts():
            ...
            if source_geneProduct.getId() not in targetGenProductID:
                ...
                rpSBML.checklibSBML(
                    target_geneProduct.setId(source_geneProduct.getId()),
                    "setting target gene product id",
                )
                rpSBML.checklibSBML(
                    target_geneProduct.setLabel(source_geneProduct.getLabel()),
                    "setting target gene product label",
                )
                rpSBML.checklibSBML(
                    target_geneProduct.setName(source_geneProduct.getName()),
                    "setting target gene product name",
                )
                rpSBML.checklibSBML(
                    target_geneProduct.setMetaId(source_geneProduct.getMetaId()),
                    "setting target gene product meta_id",
                )

        # return targetGenProductID

    def copyFBCObjectives(self, source_rpsbml: "rpSBML") -> None:
        ...
        for source_objective in source_fbc.getListOfObjectives():
            ...
            if source_objective.getId() not in targetObjectiveID:
                ...
                rpSBML.checklibSBML(
                    target_objective.setId(source_objective.getId()),
                    "setting target objective",
                )
                rpSBML.checklibSBML(
                    target_objective.setName(source_objective.getName()),
                    "setting target objective",
                )
                rpSBML.checklibSBML(
                    target_objective.setType(source_objective.getType()),
                ...
                for source_fluxObjective in source_objective.getListOfFluxObjectives():
                    ...
                    rpSBML.checklibSBML(
                        target_fluxObjective, "creating target flux objective"
                    )
                    rpSBML.checklibSBML(
                        target_fluxObjective.setName(source_fluxObjective.getName()),
                ...
                rpSBML.checklibSBML(
                    target_objective.setAnnotation(source_objective.getAnnotation()),
    ...
    def copySpecies(
        ...
        species_ids = [
            spe.getId() for spe in list(source_sbml.getModel().getListOfSpecies())
        ]
        corr_species, miss_species = self.speciesMatchWith(
            species_ids=species_ids, compartment_id=compartment_id
        )
        ...
        source_species_ids = [
            spe.id for spe in source_sbml.getModel().getListOfSpecies()
        ]
        ...
        for source_spe_id in source_species_ids:
            ...
            if source_spe_id in corr_species:
                ...
                rpSBML.checklibSBML(
                    source_member, f"Retrieving the source species: {source_spe_id}"
                )
                if target_member.getAnnotation() is not None:
                    self.logger.debug(
                        f"Found annotation for target species: {corr_species[source_spe_id]}"
                    )
                    rpSBML.checklibSBML(
                    ...
                    self.logger.debug(
                        f"No annotation found for target species: {corr_species[source_spe_id]}"
                    )
            ...
            # with the corresponding compartment:
                ...
                if not source_spe:
                    ...
                    rpSBML.checklibSBML(source_spe, "fetching source species")
                    target_spe = self.getModel().createSpecies()
                    rpSBML.checklibSBML(target_spe, "creating species")
                    rpSBML.checklibSBML(
                        target_spe.setMetaId(source_spe.getMetaId()),
                        "setting target metaId",
                    )
                    ...
                    rpSBML.checklibSBML(
                        target_spe.setId(source_spe.getId()), "setting target id"
                    )
                    rpSBML.checklibSBML(
                        target_spe.setCompartment(compartment_id),
                    ...
                    rpSBML.checklibSBML(
                        target_spe.setConstant(source_spe.getConstant()),
                        "setting target constant",
                    )
                    rpSBML.checklibSBML(
                        target_spe.setAnnotation(source_spe.getAnnotation()),
                        "setting target annotation",
                    )
    ...
    def reactionsInBoth(
        ...
        for rxn_1 in rpsbml_1.getModel().getListOfReactions():
            ...
            for rxn_2 in rpsbml_2.getModel().getListOfReactions():
                match = rpSBML.reactionsAreEqual(species, rxn_1, rxn_2, logger=logger)
                if match:
                    reactions_in_both[rxn_1.getId()] = rxn_2.getId()

        return reactions_in_both

    def copyReactions(self, source_sbml: "rpSBML", species: Dict) -> Dict:
        ...
        def copySpecies(
            ...
            source_species_l = getattr(
                source_reaction, f"getListOf{spe_type.capitalize()}s"
            )()
            for source_species in source_species_l:
                src_spe_id = source_species.species
                target_species = getattr(
                    target_reaction, f"create{spe_type.capitalize()}"
                )()
                rpSBML.checklibSBML(target_species, f"create target {spe_type}")
                if src_spe_id in species:
                    rpSBML.checklibSBML(
                        target_species.setSpecies(species[src_spe_id]),
                        f"assign {spe_type} species",
                    )
                else:
                    rpSBML.checklibSBML(
                        target_species.setSpecies(src_spe_id),
                        f"assign {spe_type} species",
                    )
                rpSBML.checklibSBML(source_species, f"fetch source {spe_type}")
                rpSBML.checklibSBML(
                    target_species.setConstant(source_species.getConstant()),
                    'set "constant" on species ' + str(source_species.getConstant()),
                )
                rpSBML.checklibSBML(
                    target_species.setStoichiometry(source_species.getStoichiometry()),
        ...
        def copyProducts(
            ...
            for source_reaction_productID in [
                i.species for i in source_reaction.getListOfProducts()
            ]:
                    ...
                    if species[source_reaction_productID]:
                        if len(species[source_reaction_productID]) > 1:
                            logger.warning(
                                ...
                                + str(source_reaction_productID)
                                + ": "
                                + str(species[source_reaction_productID])
                            )
                        ...
                        rpSBML.checklibSBML(
                            target_product.setSpecies(source_reaction_productID),
                            "assign reactant product",
                        )
                else:
                    rpSBML.checklibSBML(
                ...
                rpSBML.checklibSBML(
                    target_product.setConstant(source_product.getConstant()),
                    'set "constant" on product ' + str(source_product.getConstant()),
                )
                rpSBML.checklibSBML(
                    target_product.setStoichiometry(source_product.getStoichiometry()),
        ...
        def setParameters(
            ...
            rpSBML.checklibSBML(
                target_fbc.setUpperFluxBound(source_upperFluxBound),
            ...
            rpSBML.checklibSBML(
                target_fbc.setLowerFluxBound(source_lowerFluxBound),
                "setting lower flux bound",
            )
            rpSBML.checklibSBML(
                target_reaction.setId(source_reaction.getId()), "set reaction id"
            )
            rpSBML.checklibSBML(
                target_reaction.setName(source_reaction.getName()), "set name"
            )
            rpSBML.checklibSBML(
                target_reaction.setSBOTerm(source_reaction.getSBOTerm()),
                "setting the reaction system biology ontology (SBO)",
            )  # set as process
            # TODO: consider having the two parameters as input to the function
            rpSBML.checklibSBML(
                target_reaction.setReversible(source_reaction.getReversible()),
                "set reaction reversibility flag",
            )
            rpSBML.checklibSBML(
                target_reaction.setFast(source_reaction.getFast()),
                'set reaction "fast" attribute',
            )
            rpSBML.checklibSBML(
                target_reaction.setMetaId(source_reaction.getMetaId()),
                "setting species meta_id",
            )
            rpSBML.checklibSBML(
                target_reaction.setAnnotation(source_reaction.getAnnotation()),
        ...
        for source_reaction in source_sbml.getModel().getListOfReactions():
            ...
            for target_reaction in self.getModel().getListOfReactions():
                ...
                # )
                if rpSBML.reactionsAreEqual(source_reaction, target_reaction):
    ...
    def copyGroups(self, source_sbml: "rpSBML", species: Dict, reactions: Dict) -> None:
        ...
        rpSBML.checklibSBML(
            source_sbml_doc.setPackageRequired("groups", False),
        ...
        for source_group in source_groups.getListOfGroups():
            ...
            # if the group already exists in the target then need to add new members
                ...
                target_group_ids = [
                    i.getIdRef() for i in target_group.getListOfMembers()
                ]
                for member in source_group.getListOfMembers():
    ...
    def getGroup(self, group_id: str) -> libsbml.Group:
    ...
    def copyTitles(
        self, source_sbml: "rpSBML", logger: Logger = getLogger(__name__)
    ) -> None:
        ...
        self.getModel().setName(
            ...
            + " merged with "
            + source_sbml_doc.getModel().getId()
        )
                ...
                if len(top_row) == 1:
                    ...
                    if max(row) >= x[top_row, col]:
                        logger.warning(
                            "For col "
                            + str(col)
                            + " there are either better or equal values: "
                            + str(row)
                        )
                        logger.warning(x)
                        continue
            ...
            if not np.count_nonzero(x[:, col]) == 0:
                ...
                if len(top_rows) == 1:
                    ...
                    if col_name not in to_ret:
                        ...
                        logger.warning(
                            "At this point should never have only one: "
                            + str(x[:, col])
                        )
                        logger.warning(x)
                else:
    ...
    def speciesMatchWith(
        self, species_ids: List[str], compartment_id: str
        ...
        for rxn in self.getModel().getListOfReactions():
            for spe in compartment_species:
                ...
                if spe.getId() in [
                    reactant.species for reactant in rxn.getListOfReactants()
                ] or spe.getId() in [
                    product.species for product in rxn.getListOfProducts()
                ]:
        ...
        for species_id_to_match in species_ids:
            ...
            for model_species in compartment_species:
                ...
                self.logger.debug(
                    f"species_id_to_match/compartment: {species_id_to_match}/{compartment_id}; model_species/compartment: {model_species.getId()}/{model_species.getCompartment()}"
                )
    ...
    def matchWithMIRIAM(
        ...
        miriam_annot = rpSBML.readMIRIAMAnnotation(
            model_species.getAnnotation(), logger
        )
        ...
        try:
            ...
            for db, xrefs in annot_dict.items():
                ...
                if species_id_to_match in xrefs:
                    ...
                    logger.debug(
                        f"Found match: {species_id_to_match} -> {curr_match} (involved in {curr_score} reactions)"
                    )
    ...
    def is_boundary_type(
        self, reaction: libsbml.Reaction, boundary_type: str, external_compartment: str
    ) -> bool:
        ...
        if reaction_compartment is None or reaction_compartment == "":
            ...
            if len(reactants) == 1:
                ...
                reaction_compartment = (
                    self.getModel().getSpecies(specie_id).getCompartment()
                )
        ...
        if boundary_type == "exchange":
            if (
                ...
                and len(reaction.getListOfReactants()) == 1
            ) or (
                len(reaction.getListOfProducts()) == 1
                and len(reaction.getListOfReactants()) == 0
            ):
                is_boundary = True
        ...
        return is_boundary and to_exclude and correct_compartment and rev_type

    def build_exchange_reaction(
        model: "rpSBML", compartment_id: str, logger: Logger = getLogger(__name__)
    ) -> pd_DataFrame:
        ...
        def _specie_id_from_exchange(
            reaction: libsbml.Reaction, logger: Logger = getLogger(__name__)
        ) -> str:
        ...
        for reaction in model.getModel().getListOfReactions():
            if model.is_boundary_type(reaction, "exchange", compartment_id):
                compound = pd_DataFrame(
                    dict(
                        model_id=_specie_id_from_exchange(reaction, logger),
    ...
    def compareEC(self, meas_reac_miriam, sim_reac_miriam):
        ...
        if "ec-code" in meas_reac_miriam and "ec-code" in sim_reac_miriam:
            measured_frac_ec = [
                [y for y in ec.split(".") if not y == "-"]
                for ec in meas_reac_miriam["ec-code"]
            ]
            sim_frac_ec = [
                [y for y in ec.split(".") if not y == "-"]
                for ec in sim_reac_miriam["ec-code"]
            ]
            ...
            self.logger.warning(
                f"One of the two reactions does not have any EC entries.\nMeasured: {meas_reac_miriam} \nSimulated: {sim_reac_miriam}"
            )
            return 0.0
    ...
    def _get_reactions_with_species_keys(
        self, chem_repr=["inchikey", "inchi", "smiles"], group_id: str = "rp_pathway"
    ) -> Dict:
        """
        ...
        for rxn_id in self.readGroupMembers(group_id):
            ...
            reactions[rxn_id] = rpSBML.readBRSYNTHAnnotation(
                reaction.getAnnotation(), logger=self.logger
            )
        ...
        for spe in self.getModel().getListOfSpecies():
            species[spe.getId()] = rpSBML.readBRSYNTHAnnotation(
                spe.getAnnotation(), logger=self.logger
            )
        ...
        for reaction_id in reactions:
            ...
            d_reactants = {}
            for reactant in (
                self.getModel().getReaction(reaction_id).getListOfReactants()
                ...
                key = (
                    species[reactant.getSpecies()][key]
                    if key
                    else reactant.getSpecies()
                )
            ...
            for product in self.getModel().getReaction(reaction_id).getListOfProducts():
                ...
                key = (
                    species[product.getSpecies()][key] if key else product.getSpecies()
                )
    ...
    def getPlugin(self, plugin: str) -> object:
        ...
        self.checklibSBML(plug, "Getting {} package".format(plugin))
        return plug
    ...
    #######################################################################
    @staticmethod
    def checklibSBML(value, message: str, logger: Logger = getLogger(__name__)) -> int:
    ...
    def _compareXref(self, current, toadd):
        ...
        for database_id in current:
            try:
                list_diff = [
                    i for i in _toadd[database_id] if i not in current[database_id]
                ]
                if not list_diff:
    ...
    def updateBRSynth(self, sbase_obj, annot_header, value, meta_id=None):
        ...
        if not annot_obj:
            self.logger.error(
                f"Cannot convert this string to annotation object: {annotation}"
            )
            return False
        ...
        if not obj_annot:
            sbase_obj.setAnnotation(
                libsbml.XMLNode.convertStringToXMLNode(
                    self._defaultBRSynthAnnot(meta_id)
                )
            )
            obj_annot = sbase_obj.getAnnotation()
            if not obj_annot:
                ...
                return False
        brsynth_annot = (
            obj_annot.getChild("RDF").getChild("BRSynth").getChild("brsynth")
        )
    ...
    def updateBRSynthAnnot(self, annot, annot_obj, annot_header):
        ...
        for i in range(annot.getNumChildren()):
            ...
            if annot_header == annot.getChild(i).getName():
                ...
                source_found = False
                source_annot = (
                    annot_obj.getChild("RDF").getChild("BRSynth").getChild("brsynth")
                )
                ...
                for y in range(source_annot.getNumChildren()):
                    ...
                    if str(annot_header) == str(source_annot.getChild(y).getName()):
                        ...
                        self.checklibSBML(
                            annot.addChild(towrite_annot),
                            " 1 - Adding annotation to the brsynth annotation",
                            self.logger,
                        )
                        break
                ...
                if not source_found:
                    self.logger.error(
                        f"Cannot find {annot_header} in source annotation"
                    )
    ...
    def addBRSynthAnnot(self, brsynth_annot, annot_obj, annot_header):
        ...
        for y in range(source_brsynth_annot.getNumChildren()):
            ...
            if str(annot_header) == str(source_brsynth_annot.getChild(y).getName()):
                ...
                self.checklibSBML(
                    brsynth_annot.addChild(towrite_annot),
                    "2 - Adding annotation to the brsynth annotation",
                    self.logger,
                )
                break
    ...
    def addUpdateMIRIAM(self, sbase_obj, type_param, xref, meta_id=None):
        ...
        if type_param not in ["compartment", "reaction", "species"]:
            self.logger.error(
                f"type_param must be ['compartment', 'reaction', 'species'] not {type_param}"
            )
            return False
        miriam_annot = None
        isReplace = False
        try:
            ...
            if not miriam_elements:
                ...
                miriam_annot_1 = libsbml.XMLNode.convertStringToXMLNode(
                    self._defaultBothAnnot(meta_id)
                )
                miriam_annot = (
                    miriam_annot_1.getChild("RDF")
                    .getChild("Description")
                    .getChild("is")
                    .getChild("Bag")
                )
            ...
            try:
                ...
                if not meta_id:
                    meta_id = self._genMetaID("tmp_addUpdateMIRIAM")
                miriam_annot = libsbml.XMLNode.convertStringToXMLNode(
                    self._defaultMIRIAMAnnot(meta_id)
                )
                miriam_annot = (
                    miriam_annot.getChild("RDF")
                    .getChild("Description")
                    .getChild("is")
                    .getChild("Bag")
                )
        ...
        for i in range(miriam_annot.getNumChildren()):
            ...
            if single_miriam.getAttributes().getLength() > 1:
                self.logger.error(
                    f"MIRIAM annotations should never have more than 1: {single_miriam.toXMLString()}"
                )
                continue
            ...
            if not single_miriam_attr.isEmpty():
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Cannot find the self.header_miriram entry {db}"
                        )
                        continue
            else:
      ...
      <bqbiol:is>
        ...
        if isReplace:
            ...
            if not ori_miriam_annot:
                ...
                rpSBML.checklibSBML(
                    ori_miriam_annot.getChild("RDF")
                    .getChild("Description")
                    .getChild("is")
                    .removeChild(0),
                    'Removing annotation "is"',
                    self.logger,
                )
                rpSBML.checklibSBML(
                    ori_miriam_annot.getChild("RDF")
    ...
    def search_objective_from_rxnid(self, rxn_id: str) -> str:
    ...
    def search_objective(self, objective_id: str, reactions: List[str] = []) -> str:
        ...
        for objective in fbc_plugin.getListOfObjectives():
            ...
            if objective.getId() == objective_id:
                self.logger.debug(
                    f"The specified objective id {objective_id} already exists"
                )
                return objective_id

            if not set(
                [i.getReaction() for i in objective.getListOfFluxObjectives()]
            ) - set(reactions):
                # TODO: consider setting changing the name of the objective
                self.logger.debug(
                    f"The specified objective id {objective_id} has another objective with the same reactions: {objective.getId()}"
                )
                return objective.getId()
    ...
    def create_multiflux_objective(
        ...
        if len(reactionNames) != len(coefficients):
            self.logger.error(
                "The size of reactionNames is not the same as coefficients"
            )
            exit()
        ...
        fbc_plugin.setActiveObjectiveId(
            fluxobj_id
        )  # this ensures that we are using this objective when multiple
    ...
    def readSBML(self, inFile):
        ...
        for err in [
            self.getDocument().getError(i)
            for i in range(self.getDocument().getNumErrors())
        ]:
        ...
        if not self.getModel():
            self.logger.error(
                "Either the file was not read correctly or the SBML is empty"
            )
            raise FileNotFoundError
        ...
        if not self.getModel().isPackageEnabled("groups"):
            ...
            rpSBML.checklibSBML(
                self.getDocument().setPackageRequired("groups", False),
                "enabling groups package",
            )
        if not self.getModel().isPackageEnabled("fbc"):
            rpSBML.checklibSBML(
                self.getModel().enablePackage(
                    "http://www.sbml.org/sbml/level3/version1/fbc/version2", "fbc", True
                ),
                "Enabling the FBC package",
            )
            rpSBML.checklibSBML(
                self.getDocument().setPackageRequired("fbc", False),
    ...
    def readReactionSpecies(self, reaction):
        ...
        for i in range(reaction.getNumReactants()):
            ...
            toRet["left"][reactant_ref.getSpecies()] = int(
                reactant_ref.getStoichiometry()
            )

        # products
        for i in range(reaction.getNumProducts()):
            product_ref = reaction.getProduct(i)
            toRet["right"][product_ref.getSpecies()] = int(
                product_ref.getStoichiometry()
            )
    ...
    def readRPspecies(self, pathway_id="rp_pathway"):
        ...
        for reacId in self.readGroupMembers(pathway_id):
            ...
            for pro in reac.getListOfProducts():
                reacMembers[reacId]["products"][
                    pro.getSpecies()
                ] = pro.getStoichiometry()
            for rea in reac.getListOfReactants():
                reacMembers[reacId]["reactants"][
                    rea.getSpecies()
                ] = rea.getStoichiometry()
        return reacMembers
    ...
    def readMIRIAMAnnotation(
        annot: libsbml.XMLNode, logger: Logger = getLogger(__name__)
    ):
    ...
    def readBRSYNTHAnnotation(
        ...
        def _readBRSYNTHAnnotationToDict(
            ...
            for i_child in range(annot.getNumChildren()):
                ...
                toRet[child.getName()] = {
                    child.getAttributes()
                    .getName(i_attr): child.getAttributes()
                    .getValue(i_attr)
        ...
        for i in range(bag.getNumChildren()):
            ...
            # Read list
            if ann.getName().startswith("tmpl_rxn_ids") or ann.getName().startswith(
                "rule_ids"
            ):
                toRet[ann.getName()] = _readBRSYNTHAnnotationToList(
                    annot=ann, logger=logger
                )
        ...
        for reaction in self.getModel().getListOfReactions():
            if reaction.getId() not in ignoreReactions:
                ...
                if reaction.reversible:
                    ...
                    if (
                        param_dict[reaction_fbc.getLowerFluxBound()] >= 0
                        and param_dict[reaction_fbc.getUpperFluxBound()] > 0
                    ):
                        if species_id in [
                            i.getSpecies() for i in reaction.getListOfProducts()
                        ]:
                            return True
                    # can go both ways
                    elif (
                        ...
                        if species_id in [
                            ...
                            i.getSpecies() for i in reaction.getListOfReactants()
                        ]:
                            return True
                    # strict right to left
                        ...
                        param_dict[reaction_fbc.getLowerFluxBound()] < 0
                        and param_dict[reaction_fbc.getUpperFluxBound()] <= 0
                        and param_dict[reaction_fbc.getLowerFluxBound()]
                        < param_dict[reaction_fbc.getUpperFluxBound()]
                    ):
                        if species_id in [
                            i.getSpecies() for i in reaction.getListOfReactants()
                        ]:
                        ...
                        self.logger.warning(
                            f"isSpeciesProduct does not find the directionailty of the reaction for reaction: {species_id}"
                        )
                        return True
                else:
    ...
    def read_reactions(
        self, pathway_id: str = None, logger: Logger = getLogger(__name__)
    ) -> Dict:
        """
    ...
    def read_reaction(self, rxn_id: str, logger: Logger = getLogger(__name__)) -> Dict:
        """
    ...
    def read_species(self, logger: Logger = getLogger(__name__)) -> Dict:
        """
        ...
        for species in self.getModel().getListOfSpecies():
            ...
            species_dict[species.getId()]["brsynth"] = self.readBRSYNTHAnnotation(
                annot, self.logger
            )
    ...
    #########################################################################
    @staticmethod
    def from_json(filename: str, logger: Logger = getLogger(__name__)) -> "rpSBML":

        with open(filename, "r") as fp:
            pathway = json_load(fp)

        return rpSBML.from_dict(pathway=pathway, logger=logger)
    ...
    def create_enriched_group(
        ...
        for key, value in infos.items():
            self.updateBRSynth(
                sbase_obj=self.getGroup("rp_pathway"), annot_header=key, value=value
            )

    def to_cobra(self, logger: Logger = getLogger(__name__)) -> cobra.Model:
        ...
        with NamedTemporaryFile(delete=False) as temp_f:
            ...
            try:
                cobra_model = cobra_io.read_sbml_model(
                    temp_f.name, use_fbc_package=True
                )
    ...
    @staticmethod
    def from_cobra(
        model: cobra.Model, logger: Logger = getLogger(__name__)
    ) -> "rpSBML":
        ...
        # cobra_model = None
        with NamedTemporaryFile(delete=False) as temp_f:
            cobra.io.write_sbml_model(model, temp_f.name)
            temp_f.close()

        rpsbml = rpSBML(inFile=temp_f.name, logger=logger)
        # To handle file removing (Windows)
        remove(temp_f.name)
    ...
    def compareMIRIAMAnnotations(
        source_annot, target_annot, logger: Logger = getLogger(__name__)
    ) -> bool:
    ...
    def convert_miriam_to_dict(miriam: List) -> Dict:
        ...
        for xref in miriam:
            try:
                m = re.match(
                    "^http(s)?://identifiers.org/(\w+\.?\w+)[:/](\w+:?\w*)", xref
                ).groups()
                db = m[1]
                ref = m[2]
                if db in res_dict:
    ...
    ############################# MODEL APPEND ##############################
    #########################################################################
    def getReactionConstraints(self, rxn_id: str) -> Tuple[float, float]:
        """
        Returns flux bounds.
        ...
        self.logger.debug(
            f"Getting lower flux bound parameter: {reac_fbc.getLowerFluxBound()}"
        )
        self.logger.debug(
            f"Getting upper flux bound parameter: {reac_fbc.getUpperFluxBound()}"
        )
    ...
    def setReactionConstraints(
        ...
        rpSBML.checklibSBML(
            reac_fbc.setUpperFluxBound(upper_param.getId()),
        ...
        rpSBML.checklibSBML(
            reac_fbc.setLowerFluxBound(lower_param.getId()),
    ...
    def createModel(self, name="dummy", model_id="dummy", meta_id=None):
        ...
        rpSBML.checklibSBML(
            self.sbmlns.addPkgNamespace("groups", 1), "Add groups package"
        )
        ...
        #!!!! must be set to false for no apparent reason
        rpSBML.checklibSBML(
            self.document.setPackageRequired("fbc", False), "enabling FBC package"
        )
        #!!!! must be set to false for no apparent reason
        rpSBML.checklibSBML(
            self.document.setPackageRequired("groups", False), "enabling groups package"
        )
        ## sbml model
        ...
        rpSBML.checklibSBML(
            self.getModel().setTimeUnits("second"), "setting model time unit"
        )
        rpSBML.checklibSBML(
            self.getModel().setExtentUnits("mole"), "setting model compartment unit"
        )
        rpSBML.checklibSBML(
            self.getModel().setSubstanceUnits("mole"), "setting model substance unit"
        )
    ...
    def createCompartment(self, size, compId, compName, compXref, meta_id=None):
        ...
        rpSBML.checklibSBML(
            comp.setSBOTerm(290), "set SBO term for the cytoplasm compartment"
        )
        if not meta_id:
            meta_id = self._genMetaID(compId)
        rpSBML.checklibSBML(
            comp.setMetaId(meta_id), "set the meta_id for the compartment"
        )
        ############################ MIRIAM ############################
        comp.setAnnotation(
            libsbml.XMLNode.convertStringToXMLNode(self._defaultMIRIAMAnnot(meta_id))
        )
    ...
    def createUnit(self, unitDef, libsbmlunit, exponent, scale, multiplier):
        ...
        rpSBML.checklibSBML(
            unit.setExponent(exponent), "setting the exponenent of the unit"
        )
        rpSBML.checklibSBML(unit.setScale(scale), "setting the scale of the unit")
        rpSBML.checklibSBML(
            unit.setMultiplier(multiplier), "setting the multiplier of the unit"
        )
    ...
    def createReturnFluxParameter(
        ...
        if param_id in [i.getId() for i in self.getModel().getListOfParameters()]:
            ...
            rpSBML.checklibSBML(
                newParam.setConstant(is_constant), "setting as constant"
            )
    ...
    def createReaction(
        ...
        ## BOUNDS
        # Upper
        upper_bound = self.createReturnFluxParameter(value=fbc_upper, units=fbc_units)
        rpSBML.checklibSBML(
            reac_fbc.setUpperFluxBound(upper_bound.getId()),
            f"setting {str(id)} upper flux bound",
        )
        # Lower
        lower_bound = self.createReturnFluxParameter(value=fbc_lower, units=fbc_units)
        rpSBML.checklibSBML(
            reac_fbc.setLowerFluxBound(lower_bound.getId()),
        ...
        # reactions
        rpSBML.checklibSBML(
            reac.setId(id), "set reaction id"
        )  # same convention as cobrapy
        rpSBML.checklibSBML(
            reac.setSBOTerm(176), "setting the system biology ontology (SBO)"
        )  # set as process
        # TODO: consider having the two parameters as input to the function
        rpSBML.checklibSBML(
            reac.setReversible(reversible), "set reaction reversibility flag"
        )
        ...
        for spe_id, spe_sto in reactants.items():
            ...
            reactant_ext = spe_id
            rpSBML.checklibSBML(
                spe.setSpecies(str(reactant_ext)), "assign reactant species"
            )
            # TODO: check to see the consequences of heterologous parameters not being constant
            rpSBML.checklibSBML(
                spe.setConstant(True), 'set "constant" on species ' + str(reactant_ext)
            )
            rpSBML.checklibSBML(
        ...
        for spe_id, spe_sto in products.items():
            ...
            product_ext = spe_id
            rpSBML.checklibSBML(
                pro.setSpecies(str(product_ext)), "assign product species"
            )
            # TODO: check to see the consequences of heterologous parameters not being constant
            rpSBML.checklibSBML(
                pro.setConstant(True), 'set "constant" on species ' + str(product_ext)
            )
            rpSBML.checklibSBML(
                pro.setStoichiometry(float(spe_sto)),
                "set the stoichiometry (" + str(float(spe_sto)) + ")",
            )

        ############################ MIRIAM ############################
        rpSBML.checklibSBML(
            reac.setAnnotation(self._defaultBothAnnot(meta_id)), "creating annotation"
        )
        self.addUpdateMIRIAM(reac, "reaction", reacXref, meta_id)
        ###### BRSYNTH additional information ########
        self.updateBRSynth(
            sbase_obj=reac, annot_header="smiles", value=smiles, meta_id=meta_id
        )
        for key, value in infos.items():
            ...
            self.updateBRSynth(sbase_obj=reac, annot_header=key, value=value)
    ...
    def createSpecies(
        ...
        rpSBML.checklibSBML(
            spe.setBoundaryCondition(is_boundary), "set boundary conditions"
        )
        rpSBML.checklibSBML(spe.setConstant(False), "set constant")
        # useless for FBA (usefull for ODE) but makes Copasi stop complaining
        rpSBML.checklibSBML(
            spe.setInitialConcentration(1.0), "set an initial concentration"
        )
        ...
        if not species_name:
            rpSBML.checklibSBML(
                spe.setName(species_id),
                "setting name for the metabolite " + str(species_id),
            )
        else:
            rpSBML.checklibSBML(
                spe.setName(species_name),
                "setting name for the metabolite " + str(species_name),
            )
        if compartment:
        ...
        rpSBML.checklibSBML(
            spe.setAnnotation(self._defaultBothAnnot(meta_id)), "creating annotation"
        )
        # Annotations.
        self.addUpdateMIRIAM(spe, "species", chemXref, meta_id)
        if smiles is not None:
            self.updateBRSynth(
                sbase_obj=spe, annot_header="smiles", value=smiles, meta_id=meta_id
            )
        if inchi is not None:
            self.updateBRSynth(
                sbase_obj=spe, annot_header="inchi", value=inchi, meta_id=meta_id
            )
        if inchikey is not None:
            self.updateBRSynth(
                sbase_obj=spe, annot_header="inchikey", value=inchikey, meta_id=meta_id
            )
        for key, value in infos.items():
            self.updateBRSynth(
                sbase_obj=spe, annot_header=key, value=value, meta_id=meta_id
            )

    def createGroup(
        self, id: str, brs_annot: bool = True, meta_id: str = None
    ...
    def removeGroup(self, id: str) -> libsbml.GroupsModelPlugin:
        """
    ...
    def addMember(self, group_id: str, idRef: str) -> None:
        group = self.getGroup(group_id)
        member = libsbml.Member()
    ...
    def createFluxObj(
        self, fluxobj_id, reactionName, coefficient, isMax=True, meta_id=None
    ):
        """Create libSBML flux objective
    ...
    def activateObjective(self, objective_id: str, plugin: str = "fbc") -> None:
    ...
    def genericModel(self, modelName, modelID, compartments, unit_def):
            ...
            if type(comp["annot"]) is dict:
                for db, comps in comp["annot"].items():
                    for comp_name in comps:
                        comp_lst += [
                            f"http://identifiers.org/{self.miriam_header['compartment'][db]}{comp_name}"
                        ]
            else:
                comp_lst = comp["annot"]
