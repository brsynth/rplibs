...
class rpGraph:
    ...
    def __init__(
        ...
        if rpsbml:
            self.__makeGraph(
                is_gem_sbml, pathway_id, central_species_group_id, sink_species_group_id
            )
    ...
    def __makeGraph(
        self, is_gem_sbml, pathway_id, central_species_group_id, sink_species_group_id
    ):
        ...
        self.G = nx.DiGraph(
            brsynth=self.rpsbml.readBRSYNTHAnnotation(
                annot=rp_pathway.getAnnotation(), logger=self.logger
            )
        )
        ...
        for species in rpsbml_model.getListOfSpecies():
            ...
            if is_rp_pathway or is_gem_sbml:
                ...
                self.G.add_node(
                    ...
                    brsynth=self.rpsbml.readBRSYNTHAnnotation(
                        annot=species.getAnnotation(), logger=self.logger
                    ),
        ...
        for reaction in rpsbml_model.getListOfReactions():
            ...
            if reaction.getId() in rp_reactions_id or is_gem_sbml:
                for reac in reaction.getListOfReactants():
                    ...
                    self.G.add_edge(
                        reac.species, reaction.getId(), stoichio=reac.stoichiometry
                    )
                for prod in reaction.getListOfProducts():
                    ...
                    self.G.add_edge(
                        reaction.getId(), prod.species, stoichio=reac.stoichiometry
                    )
        ...
        for spe_id in species:
            ...
            if node["type"] == "species":
                if node["rp_pathway"]:
                    ...
                    if (
                        (only_central and node["rp_trunk_species"])
                        or (only_rp_pathway and node["rp_pathway"])
                        or (not only_central and not only_rp_pathway)
                    ):
            ...
            for n_n in succ_node_list:
                ...
                if n["type"] == "reaction":
                    if n_n in multi_reac:
                        self._recursiveReacSuccessors(
                            n_n, current_reac_list, all_res, num_reactions
                        )
                elif n["type"] == "species":
    ...
    def _recursiveReacPredecessors(self, node_name, reac_list, all_res, num_reactions):
        ...
        if len(flat_reac_list) == num_reactions:
            ...
            for n_n in pred_node_list:
                ...
                if n["type"] == "reaction":
                    if n_n in multi_reac:
                        self._recursiveReacPredecessors(
                            n_n, current_reac_list, all_res, num_reactions
                        )
                elif n["type"] == "species":
                    if n["rp_trunk_species"]:
    ...
    def orderedRetroReactions(self):
        ...
        for cons_cent_spe in self._onlyConsumedSpecies():
            res = self._recursiveReacSuccessors(
                cons_cent_spe, [], [], self.num_reactions
            )
            if res:
        ...
        for prod_cent_spe in self._onlyProducedSpecies():
            res = self._recursiveReacPredecessors(
                prod_cent_spe, [], [], self.num_reactions
            )
            if res:
                self.logger.debug(res)
                if len(res) == 1:
