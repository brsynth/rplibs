...
class Test_rpSBML(TestCase):
    ...
    def test__to_dict_empty(self):
        self.assertDictEqual(self.rpobject_empty._to_dict(), {})
    ...
    def test___eq__(self):
        ...
        self.assertEqual(self.rpobject, rpobject)

    def test_not__eq__(self):
        # test type
        self.assertNotEqual(self.rpobject, 1)
        # test with empty rpObject
        self.assertNotEqual(self.rpobject, self.rpobject_empty)
        # test by modifying a value
        rpobject = deepcopy(self.rpobject)
        fba = deepcopy(self.__fba)
        fba["value"] += 1
        rpobject.add_fba_info(self.__key, fba)
        self.assertNotEqual(self.rpobject, rpobject)
    ...
    def test_get_thermo(self):
        self.assertDictEqual(self.rpobject.get_thermo(), {self.__key: self.__thermo})

    def test_get_thermo_info(self):
        self.assertDictEqual(self.rpobject.get_thermo_info(self.__key), self.__thermo)

    def test_get_fba_info(self):
        self.assertDictEqual(self.rpobject.get_fba_info(self.__key), self.__fba)

    def test_get_fba(self):
        self.assertDictEqual(self.rpobject.get_fba(), {self.__key: self.__fba})
    ...
    def test_get_thermo_attr(self):
        ...
        for attr in attrs:
            with self.subTest(f"Testing get/set thermo_{attr}()", attr=attr):
                ...
                self.assertEqual(
                    getattr(self.rpobject_empty, f"get_thermo_{attr}")(), value
                )
    ...
    def test_get_fba_attr(self):
        ...
        for attr in attrs:
            with self.subTest(f"Testing get/set fba_{attr}()", attr=attr):
                ...
                self.assertEqual(
                    getattr(self.rpobject_empty, f"get_fba_{attr}")(), value
                )
