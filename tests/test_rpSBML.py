...
class Test_rpSBML(Main_rplibs):
    ...
    # )
    # retModel(self):
    # def getDocumepsbml_json = os_path.join(
    #    Main_rplibs.data_path,
    #    'rpsbml.json'
    # )
    ...
    def setUp(self):
        ...
        self.rpsbml_lycopene = rpSBML(
            inFile=self.rpsbml_lycopene_path, logger=self.logger
        )
    ...
    def test_formatId(self):
        self.assertEqual(
            rpSBML.formatId("2-Oxoglutarate+"), "_2_Oxoglutarate_ASCII_43_ASCII_"
        )
    ...
    def test_getModel(self):
        self.assertIsInstance(self.rpsbml_lycopene.getModel(), libsbml.Model)
        self.assertFalse(self.rpsbml_none.getModel())

    def test_getDocument(self):
        self.assertIsInstance(self.rpsbml_lycopene.getDocument(), libsbml.SBMLDocument)
        self.assertFalse(self.rpsbml_none.getDocument())

    def test_getName(self):
        self.assertEqual(self.rpsbml_lycopene.getName(), self.rpsbml_lycopene_name)
        self.assertEqual(self.rpsbml_none.getName(), "dummy")

    def test_setName(self):
        self.rpsbml_lycopene.setName("dummy")
        self.rpsbml_none.setName(self.rpsbml_lycopene_name)
    ...
    def test_search_compartment(self):
        res = self.rpsbml_lycopene.search_compartment(compartment="c")
        # Test - 1.
        self.assertIsInstance(res, libsbml.Compartment)
        self.assertEqual(res.getId(), "c")
        # Test - 2
        res = self.rpsbml_lycopene.search_compartment(compartment="cytosol")
        self.assertIsInstance(res, libsbml.Compartment)
        self.assertEqual(res.getId(), "c")
        # Test - 3
        res = self.rpsbml_lycopene.search_compartment(compartment="cyxosol")
    ...
    def test_has_compartment(self):
        # Test - 1
        res = self.rpsbml_lycopene.has_compartment(compartment="c", strict=True)
        self.assertEqual(res.getId(), "c")
        # Test - 2
        res = self.rpsbml_lycopene.has_compartment(compartment="C", strict=True)
        self.assertIsNone(res)
        # Test - 3
        res = self.rpsbml_lycopene.has_compartment(compartment="c")
        self.assertEqual(res.getId(), "c")
        # Test - 4
        ...
        self.assertEqual(res.getId(), "c")
        # Test - 5
        res = self.rpsbml_lycopene.has_compartment(compartment="intracellular space")
        self.assertEqual(res.getId(), "c")

    def test_search_specie(self):
        # Test - 1
        res = self.rpsbml_lycopene.search_specie(specie="MNXM8975")
        self.assertIsInstance(res, libsbml.Species)
        self.assertEqual(res.getId(), "MNXM8975")
        # Test - 2
        res = self.rpsbml_lycopene.search_specie(specie="MNTM8975")
        self.assertIs(res, None)
        # Test - 3
        res = self.rpsbml_lycopene.search_specie(specie="mnxm8975")
        self.assertEqual(res.getId(), "MNXM8975")
    ...
    def test_has_specie(self):
        # Test - 1
        res = self.rpsbml_lycopene.has_specie(specie="MNXM8975")
        ...
        self.assertTrue(res)
        # Test - 3
        res = self.rpsbml_lycopene.has_specie(specie="NMXM000")
        self.assertIsInstance(res, bool)
        self.assertFalse(res)

    def test_search_reaction(self):
        # Test - 1
        res = self.rpsbml_lycopene.search_reaction(reaction="rxn_1")
        self.assertIsInstance(res, libsbml.Reaction)
        self.assertEqual(res.getId(), "rxn_1")
        # Test - 2
        res = self.rpsbml_lycopene.search_reaction(reaction="rtn")
        self.assertIs(res, None)
        # Test - 3
        res = self.rpsbml_lycopene.search_reaction(reaction="RXN_1")
        self.assertEqual(res.getId(), "rxn_1")
    ...
    def test_has_reaction(self):
        # Test - 1
        res = self.rpsbml_lycopene.has_reaction(reaction="rxn_1")
        self.assertIsInstance(res, bool)
        self.assertTrue(res)
        # Test - 2
        res = self.rpsbml_lycopene.has_reaction(reaction="XN_1")
        self.assertIsInstance(res, bool)
        self.assertTrue(res)
        # Test - 3
        res = self.rpsbml_lycopene.has_reaction(reaction="rtn")
        self.assertIsInstance(res, bool)
        self.assertFalse(res)
    ...
    def test_speciesMatchWith(self):
        ...
        self.assertEqual(
            ...
            sorted(
                self.rpsbml_lycopene.speciesMatchWith(
                    self.rpsbml_lycopene_specie_id, "c"
                )[0].values()
            ),
        )
        # Challenge - 1
        corr_spe, miss_spe = self.rpsbml_lycopene.speciesMatchWith(
            ["HMDB00250", "CHEBI:13420"], "c"
        )
        self.assertDictEqual(
            corr_spe, {"HMDB00250": "M_ppi_c", "CHEBI:13420": "M_ppi_c"}
        )
    ...
    def test_is_boundary_type(self):
        ...
        # Load.
        rpsbml_ecoli = rpSBML(inFile=self.rpsbml_ecoli_path, logger=self.logger)
        reactions = rpsbml_ecoli.getModel().getListOfReactions()
        cobra_model = cobra_io.read_sbml_model(
            self.rpsbml_ecoli_path, use_fbs_package=True
        )
        # Return type.
        ...
        # Exchange.
        rpsbml_exchange = [
            x for x in reactions if rpsbml_ecoli.is_boundary_type(x, "exchange", "e")
        ]
        self.assertEqual(len(cobra_model.exchanges), len(rpsbml_exchange))
        rpsbml_exchange = [
            x for x in reactions if rpsbml_ecoli.is_boundary_type(x, "exchange", "")
        ]
        self.assertEqual(len(cobra_model.exchanges), len(rpsbml_exchange))
        # Demand.
        rpsbml_demands = [
            x for x in reactions if rpsbml_ecoli.is_boundary_type(x, "demand", "")
        ]
        self.assertEqual(len(cobra_model.demands), len(rpsbml_demands))
        # Sinks.
        rpsbml_sinks = [
            x for x in reactions if rpsbml_ecoli.is_boundary_type(x, "sink", "")
        ]
    ...
    def test_createSpecies(self):
        ...
        rt = self.rpsbml_lycopene.createSpecies(species_id="M_ipdp_y")
        # Return type.
        self.assertIs(rt, None)
        # Values.
        species = [x for x in self.rpsbml_lycopene.read_species()]
        self.assertEqual(len(species), 1 + len(nb_species))
        # Challenge - create
        species_id = "M_ipdp_c"
        ...
        self.assertEqual(species_name, specie.getName())
        ...
        self.assertEqual(compartment, specie.getCompartment())
        self.assertEqual(metaid, specie.getMetaId())
        ...
        species = [
            x
            for x in self.rpsbml_lycopene.getModel().getListOfSpecies()
            if x.getId() == species_id
        ]
    ...
    def test_isolated_species(self):
        ...
        self.assertSetEqual(
            set(self.rpsbml_lycopene.get_isolated_species()), set(["MNXM24"])
        )
        self.rpsbml_lycopene.rm_isolated_species()
        self.assertListEqual(self.rpsbml_lycopene.get_isolated_species(), [])
    ...
    #            feat-tests-rplibs-rp                'MNXM13__64__MNXC3',
    ...
    #    ror: ['C     self.rpsbml.getModel().getSpecies('MNXM89557__64__MNXC3').getAnnotation(),
