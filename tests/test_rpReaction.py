...
class Test_rpReaction(TestCase):
    ...
    def setUp(self):
        self.reactants = {"CMPD_0000000010": 1, "MNXM1": 1}
        self.products = {"CMPD_0000000003": 1, "MNXM13": 1}
        ...
        self.specific_dict = {
            "idx_in_path": -1,
            "transfo_id": self.transfo_id,
            "rule_ids": self.rule_ids,
            "tmpl_rxn_ids": self.tmpl_rxn_ids,
            "rule_score": self.rule_score,
            **{"selenzy_" + id: self.selenzy[id] for id in self.selenzy},
        }

    ## READ METHODS
    def test_get_transfo_id(self):
        self.rxn.set_transfo_id(self.transfo_id)
        self.assertEqual(self.rxn.get_transfo_id(), self.transfo_id)

    def test_get_rule_ids(self):
        self.rxn.set_rule_ids(self.rule_ids)
        self.assertEqual(self.rxn.get_rule_ids(), self.rule_ids)

    def test_get_tmpl_rxn_ids(self):
        self.rxn.set_tmpl_rxn_ids(self.tmpl_rxn_ids)
        self.assertEqual(self.rxn.get_tmpl_rxn_ids(), self.tmpl_rxn_ids)

    def test_get_rule_score(self):
        self.rxn.set_rule_score(self.rule_score)
        self.assertEqual(self.rxn.get_rule_score(), self.rule_score)
    ...
    def test__to_dict(self):
        ...
        self.assertDictEqual(
            self.rxn._to_dict(full=True), {**self.inherited_dict, **self.specific_dict}
        )
        self.assertDictEqual(self.rxn._to_dict(full=False), self.specific_dict)
    ...
    def test_get_default_fbc(self):
        ...
        for id, val in fbc.items():
            with self.subTest(f"Testing get/set fbc_{id}()", id=id, val=val):
                ...
                self.assertEqual(getattr(self.rxn, f"get_fbc_{id}")(), val)
    ...
    def test_miriam(self):
        self.assertDictEqual(self.rxn.get_miriam(), self.miriam)

    def test_selenzy(self):
        self.rxn.set_selenzy(self.selenzy)
        self.assertDictEqual(self.rxn.get_selenzy(), self.selenzy)
        id = "UniProtID_1"
        self.assertEqual(self.rxn.get_selenzy_infos_fromID(id), self.selenzy[id])

    def test_add_miriam(self):
        db = "bigg"
        xref = "bigg_ID"
        self.rxn.add_miriam(db, xref)
        self.assertDictEqual(self.rxn.get_miriam(), {**self.miriam, **{db: xref}})
